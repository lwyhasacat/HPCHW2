  ...
  for(int block = 0; block < M*N; block++){
    ...
    for(int i = 0; i < BLOCK_SIZE; i++){
      for(int j = 0; j < BLOCK_SIZE; j++){
        C[j * BLOCK_SIZE + i] = c[m * (c2 * BLOCK_SIZE + j) + c1 * BLOCK_SIZE + i];
      }
    }
    ...
    for(int i = 0; i < K; i++){
      ...
      for(int j = 0; j < BLOCK_SIZE; j++){
        for(int l = 0; l < BLOCK_SIZE; l++){
          A[l * BLOCK_SIZE + j] = a[m * (i * BLOCK_SIZE + l) + c1 * BLOCK_SIZE + j];
        }
      }
      // B
      for(int j = 0; j < BLOCK_SIZE; j++){
        for(int l = 0; l < BLOCK_SIZE; l++){
          B[l * BLOCK_SIZE + j] = b[k * (c2 * BLOCK_SIZE + l) + i * BLOCK_SIZE + j];
        }
      }
    ...
    for(int i = 0; i < BLOCK_SIZE; i++){
      for(int j = 0; j < BLOCK_SIZE; j++){
        c[m * (c2 * BLOCK_SIZE + j) + c1 * BLOCK_SIZE + i] = C[j * BLOCK_SIZE + i];
      }
    }
  }
  aligned_free(A);
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double flops = 2 * NREPEATS * m * n * k / (time * 1e9); 
    // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = 4 * NREPEATS * m * n * k * sizeof(double) / (time * 1e9);
